...
public class _101972I {
      ...
      static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
            ...
            inv[0] = 1;
            inv[1] = 1;
            long f_temp[] = new long[100005];
            long inv_temp[] = new long[100005];
            f_temp[0]=f_temp[1]=1;
            for (int i = 2; i < 100005; i++) {
                  fac[i] = fac[i - 1] * i % mod9;
                  f_temp[i] = (mod9 - mod9/i) * f_temp[mod9 % i] % mod9;
                  inv[i] = inv[i - 1] * f_temp[i] % mod9;
                  inv_temp[i] = modpow(i,mod9-2,mod9);
            }
            pl(Arrays.toString(inv));
            pl(Arrays.toString(inv_temp));
      ...
      static long ncr(int x, int y){
            ...
            if(y>x)return 0;
            return fac[x] * inv[y] % mod9 * inv[x - y] % mod9;
      }
      ...
      static long modpow(long a, long b, long m) {
            if (b == 0) return 1;
            long p = modpow(a, b >> 1, m);
            p = p * p % m;
            return (b & 1) == 1 ? a * p % m : p ;
            //return p;
      }
