...
public class _102021B {

    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        double xc = ni(), yc = ni();
        double xd = ni(), yd = ni();
        double xb = ni(), yb = ni(), rb = ni();
        double xr = ni(), yr = ni(), rr = ni();

        double l1 = (xc - xr) * (xc - xr) + (yc - yr) * (yc - yr);
        double l2 = (xd - xr) * (xd - xr) + (yd - yr) * (yd - yr);
        double l = (xc - xd) * (xc - xd) + (yc - yd) * (yc - yd);

        double ans1 = sqrt(l1 - rr * rr);
        double ans2 = sqrt(l2 - rr * rr);

        double a1_a2_a3 = acos((l1 + l2 - l)/((sqrt(l1)*(sqrt(l2)))*2));
        double ans = a1_a2_a3 - acos(rr/sqrt(l1)) - acos(rr/sqrt(l2));
