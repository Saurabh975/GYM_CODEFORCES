...
public class _101630J {

    static LinkedList<data> adj[];
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        for (int i = 0; i < m; i++) {
            ...
            int y = ni();
            int z = ni();
            adj[x].add(new data(y, z));
            adj[y].add(new data(x, z));
            ar.add((long) z);
        }
        //pl(ar);
        ...
        ar.add(0l);
        for (long x : ar) {
            ans = min(ans, Lund_ka_bal(x) + x * k);
            //pl(ans);
        }
        pl(ans);
    }

    static long Lund_ka_bal(long w) {
        PriorityQueue<data> q = new PriorityQueue<>((data a, data b) -> a.a - b.a);
        ...
        while (!q.isEmpty()) {
            ...
            for (data data : adj[x.b]) {

                if (dis[data.a] > dis[x.b] + max(0, data.b - w)) {
                    dis[data.a] = dis[x.b] + max(0, data.b - w);
                    q.add(new data((int) dis[data.a], data.a));
                }
