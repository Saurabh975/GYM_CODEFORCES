...
public class _102501K {
    ...
    static void Mangni_ke_bail_ke_dant_na_dekhal_jye() {
        ...
        for (int i = 0; i < m; i++) {
            int x = ni(), y = ni();
            if (y == k) imp[x] = 1;
            else if (x != k) adj[y].add(x);
        }

        //for(HashSet<Integer> x : adj)pl(x);
    ...
    static void dfs(int n, int r) {
        ...
        if (vis[n].contains(r)) return;
        if (vis[n].size() >= 2) return;
